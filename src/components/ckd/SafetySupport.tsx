...
type Props = {
  language: Language;
  onRetry: () => void;
} & ({ status: "unavailable" } | { status: "risk"; saved: boolean });

export function SafetySupport(props: Props) {
    ...
    return (
      <Card className="mx-auto max-w-3xl space-y-5">
        <h1 ref={heading} tabIndex={-1} className="text-3xl font-semibold">
          {t("无法检查回答", "Couldn’t check your answer")}
        </h1>
        <p role="alert" className="text-xl">
          {t("您的回答尚未保存。请重试。", "Your answer hasn’t been saved. Please try again.")}
        </p>
        <BigButton onClick={onRetry}>{t("重试", "Retry")}</BigButton>
      </Card>
    );
  }
  ...
  return (
    <Card className="mx-auto max-w-3xl space-y-5">
      ...
      {!props.saved && <BigButton onClick={onRetry}>{t("重试", "Retry")}</BigButton>}
      <a
